...
int PositionManager::CheckOrderRequest(const Order* order) {
  ...
  if (IsOffsetClose(req->offset)) {
    int available = 0;
    auto pos = const_cast<const Portfolio*>(portfolio_)->get_position(req->contract->tid);
...
void PositionManager::OnOrderSent(const Order* order) {
  portfolio_->UpdatePending(order->req.contract->tid, order->req.direction, order->req.offset,
                            order->req.volume);
}
...
void PositionManager::OnOrderTraded(const Order* order, const Trade* trade) {
  if (trade->trade_type == TradeType::SECONDARY_MARKET ||
    ...
    portfolio_->UpdateTraded(order->req.contract->tid, order->req.direction, order->req.offset,
                             trade->volume, trade->price);
  } else if (trade->trade_type == TradeType::ACQUIRED_STOCK) {
    auto contract = ContractTable::get_by_index(trade->tid);
    assert(contract);
    portfolio_->UpdateComponentStock(contract->tid, trade->volume, true);
  } else if (trade->trade_type == TradeType::RELEASED_STOCK) {
    auto contract = ContractTable::get_by_index(trade->tid);
    assert(contract);
    portfolio_->UpdateComponentStock(contract->tid, trade->volume, false);
  }
}
