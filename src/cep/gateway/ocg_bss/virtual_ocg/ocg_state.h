...
#ifndef OCG_BSS_TEST_OCG_STATE_H_
#define OCG_BSS_TEST_OCG_STATE_H_
 ...
 public:
  ...
  void cache_early_arriving_msg(uint32_t seq, const MessageHeader& header,
    ...
    if (iter == received_messages.end()) {
      OcgCachedRecvMessage cached_msg;
  ...
  bool retrieve_msg_cache(uint32_t seq, OcgCachedRecvMessage* msg) {
    auto iter = received_messages.find(seq);
  ...
  std::unordered_map<uint32_t, OcgHistSentMessage> sent_messages;
  std::unordered_map<uint32_t, OcgCachedRecvMessage> received_messages;
};

#endif  // OCG_BSS_TEST_OCG_STATE_H_
