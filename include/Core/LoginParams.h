...
#ifndef FT_INCLUDE_CORE_LOGINPARAMS_H_
#define FT_INCLUDE_CORE_LOGINPARAMS_H_
...
class LoginParams {
 public:
  ...
  void set_api(const std::string& name) { api_ = name; }

  const std::string& front_addr() const { return front_addr_; }

  void set_front_addr(const std::string& front_addr) {
    front_addr_ = front_addr;
  }

  const std::string& md_server_addr() const { return md_server_addr_; }
  ...
  void set_broker_id(const std::string& broker_id) { broker_id_ = broker_id; }

  const std::string& investor_id() const { return investor_id_; }
  ...
  void set_passwd(const std::string& passwd) { passwd_ = passwd; }

  const std::string& auth_code() const { return auth_code_; }

  void set_auth_code(const std::string& auth_code) { auth_code_ = auth_code; }
  ...
  const auto& subscribed_list() const { return subscribed_list_; }
...
#endif  // FT_INCLUDE_CORE_LOGINPARAMS_H_
