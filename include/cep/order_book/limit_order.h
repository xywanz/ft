...
#ifndef FT_INCLUDE_CEP_ORDER_BOOK_LIMIT_ORDER_H_
#define FT_INCLUDE_CEP_ORDER_BOOK_LIMIT_ORDER_H_
...
inline uint64_t price_double2decimal(double p) {
  return (static_cast<uint64_t>(p * 100000000UL) + 50UL) / 10000UL;
}

inline double price_decimal2double(uint64_t p) {
  return static_cast<double>(p) / 10000;
}
 ...
 public:
  LimitOrder(uint64_t _id, bool _is_buy, int _volume, double _price)
      : id_(_id),
        ...
        volume_(_volume),
        price_(_price),
        decimal_price_(price_double2decimal(_price)) {}
  ...
  void set_price(double p) {
    price_ = p;
    decimal_price_ = price_double2decimal(p);
  }
...
#endif  // FT_INCLUDE_CEP_ORDER_BOOK_LIMIT_ORDER_H_
